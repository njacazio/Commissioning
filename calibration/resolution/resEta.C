void resEta(){
  ...
  TH2F *hD2 = new TH2F("hD2",";#Delta#Deltat (ps)",8,-0.8,0.8,nbins,-3000,3000);
  TH2F *hDD2 = new TH2F("hDD2","1.4 < #it{p}_{T} < 1.5 GeV/#it{c};#Delta#Deltat (ps)",8,-0.8,0.8,nbins,-3000,3000);
  TH2F *h2D2 = new TH2F("h2D2","",8,-0.8,0.8,nbins,-3000,3000);
...
//    if(!hastrd) continue;
    ...
    if(chi2 < 2) hD2->Fill(eta,dt), hDD2->Fill(eta,dt);
    else if(chi2 > 5) h2D2->Fill(eta,dt);
  }

  hRef->Fit(funcRef,"WW");

  float xx[100],ex[100],yy[100],ey[100];
  ...
  new TCanvas;
  g->Draw("AP");
  g->SetTitle("#sigma_{TOF} vs #eta; #eta; #sigma_{TOF} (ps)");
  g->SetMaximum(100);
  g->SetMinimum(50);
