void res3(){
  ...
  func->SetParLimits(4,900,1200);
  func->SetParameter(5,130);
  func->SetParLimits(5,50,300);
  func->SetParameter(6,300);
  func->SetParameter(7,20500);
  func->SetParLimits(7,20000,30000);
  ...
  TH1F *hD = new TH1F("hD","1.1 < #it{p}_{T} < 1.2 GeV/#it{c};#Delta#Deltat (ps)",1000,-3000,3000);
  TH1F *h2 = new TH1F("h2","",1000,-3000,3000);
  h2->SetLineColor(2);

  TH1F *hRef = new TH1F("hRef",";#Delta#Deltat (ps)",500,-1000,1000);

  int ibinMin = h->FindBin(2000);
  int ibinMax = h->FindBin(2400);
  ...
  for(int i=0; i < n; i++){
    ...
    if(ptUnsigned < 1.1 || ptUnsigned > 1.2) continue;
